...
cd ..\SDL_shadercross
cmake -S . -B build -G%CMAKE_GENERATOR% -DCMAKE_PREFIX_PATH=../SDL/build/ -DSDLSHADERCROSS_VENDORED=ON -DSDLSHADERCROSS_DXC=ON -DENABLE_SPIRV_CODEGEN=ON -DSDLSHADERCROSS_STATIC=ON -DSDLSHADERCROSS_CLI=ON -DSDLSHADERCROSS_CLI_STATIC=ON
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
cmake --build build --config Release
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
cd ..\..
